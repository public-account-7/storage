...
if not IsSupported then
    NZNotification.new('BB Achaotic', 'Your executor was not supported.', 10)
    return
end
...
NZNotification.new('BB Achaotic', 'Attempting to fetch game data', 5)
...
if ParryDATA.ParryFunction and ParryDATA.ParryRemote then
    NZNotification.new('BB Achaotic', 'Successfully fetched game data.', 5)
else 
    NZNotification.new('BB Achaotic', 'Failed to fetch game data, please check developer console.', 5)
end
print(string.format("[ACHAOTIC]:[DEBUG]\nParryFunction: %s\nParryRemote: %s", tostring(ParryDATA.ParryFunction), tostring(ParryDATA.ParryRemote)))

NZNotification.new('BB Achaotic', 'Attempting to load main script.', 5)
...
do
    ...
    ParrySystemSection:AddButton({
        ...
        Callback = function()
            NZNotification.new("BB Achaotic", "Attempting to re-fetch parry function, might cause a possible freeze", 5)
            local Result = AttemptFunctionFetch()
            ParryDATA.ParryFunction = Result.ParryFunction
            ParryDATA.ParryRemote = Result.ParryRemote
...
do
    ...
    for i,v in getconnections(ReplicatedStorage.Remotes.ParrySuccessClient.Event) do
        if v.Function and debug.getinfo(v.Function).name == "parrySuccessAll" then
            ...
            print("[ACHAOTIC]:[DEBUG] Found parrySuccessClient connection")
            v:Disable()
        end
    end
end
...
ClearCache = function()
    ...
    NZNotification.new('BB Achaotic', 'Cleared internal caches.', 5)
end
...
NZNotification.new('BB Achaotic', 'Loaded main script successfully.', 10)
