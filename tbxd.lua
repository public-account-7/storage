getfenv().greenlight = true --//for bypass the shit keysystem on ui lmao
...
example:AddToggle("Auto Collect [Event]", function(state)
...
local function findCar()
    ...
    for i,v in pairs(workspace.SpawnedSnowPiles:GetChildren()) do
     firetouchinterest(findCar().PrimaryPart,v,0)
      firetouchinterest(findCar().PrimaryPart,v,0)
      end
      end)
      end
    end)
    example:AddToggle("Auto Open Winter", function(state)
...
task.wait(0.2)
    ...
    example:AddToggle("Auto Open Aurora", function(state)
